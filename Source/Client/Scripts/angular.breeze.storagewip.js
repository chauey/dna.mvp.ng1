...
(function () {
    ...
    function zStorageWip($q, $rootScope, $window, zStorageConfig, zStorageCore) {
        ...
        var service = {
            ...
            storeWipEntityNew: storeWipEntityNew,
            findWipKeyByEntityName: findWipKeyByEntityName,
    };
        ...
        function findWipKeyByEntityName(entityName) {
            var wip = getWipSummary();
            //var wipItem = wip. filter(function (item) {
            //    return item.entityName.toLowerCase() === entityName.toLowerCase();
            //})[0];
            //return wipItem ? wipItem : null;
            var items = [];
            wip.forEach(function (item) {
                if (item.entityName.toLowerCase() === entityName.toLowerCase()) {
                    items.push(item);
                }
            });
        ...
        function storeWipEntityNew(entity, key, entityName, description, routeState) {
            // Entity is the entity to export.
            // key must also be passed. this allows us to save
            // an entity by itself away from the datacontext.
            // Data stashed here will not be imported into the 
        ...
        function storeWipSummary(entity, key, entityName, description, routeState) {
            var wipHeader = {
                id: entity.id || entity.typeOfTypeID, // id: entity.typeOfTypeID || {all entity id fields}
                date: new Date(),
                key: key,
